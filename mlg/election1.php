<link rel="stylesheet" href="election.css">

            <form action="elir.php" method="post">
                <div class="cont">
                    <div class="left">
                        <label for="cin">Laharan'ny karatra</label>
        <p>
            <input required="required" type="number" id="cin" name="cin">
        </p>
        
                        <label for="candidat">MPILATSAKA</label>
        <p>
            <select required="required" name="candidat">
                <option value="" class="candix">Ataovy ny safidinao</option>
                <option value="hery">RAJAONARIMAMPIANINA Hery</option>
                <option value="ravalo">RAVALOMANANA Marc</option>
                <option value="andry">ANDRY RAJOELINA</option>
            </select>
        </p>
        <p class="nommi">Ny laharan'ny kara-panondro nampidirinao dia mbola tsy voarakitra anty lisi-pifidianana,
        hamarino tsara azafady.<p>
        
                    </div>
                    <div class="right">
                    </div>
    <p class="tondro">👉</p>
        <input class="elir" type="submit" value="HIFIDY">
                </div>
            </form>


